...
from removeableObstacle import *
...
##        b = Purple (maxSpeed = self._maxSpeed // 2)
  ...
  def deleteObstacle(self, obstacle):
    self._obstacles.remove(obstacle)
    self._paper.remove(obstacle.getGraphicalRep())
...
##                        moveDir='to mouse')
  ...
  b = RedObstacle(maxSpeed=100, ballRadius=10)
  game.addObstacle(b)
  for i in range(3):
    b3 = RemoveableObstacle(maxSpeed=200, ballRadius=30)
    game.addObstacle(b3)

  b5 = PurpleObstacle(maxSpeed=200, ballRadius=5)
  game.addObstacle(b5)

  b2 = GreenObstacle (maxSpeed=50, ballRadius=20)
  game.addObstacle (b2)
  
  b4 = CyanObstacle(75, 10)
  game.addObstacle(b4)
  game.run()
